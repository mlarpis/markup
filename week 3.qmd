...
**Also known as: `Git` and GitHub**

::: callout-tip
## Lecture Week 3

This week has no slides. We discuss the flow of development with `Git` in class. You can find a 3-slide presentation exercise from week 2 [here](slides/lec-3.html).
...
As a statistician / data scientist / developer you'll need to master many skills. Some of these skills are made simple with tools. This week we'll bite the bullet by learning about a massively important tool for your toolset: **version controlling**.
...
I suggest you go full-nerd and use `Git` entirely from the terminal. All you need to do is to install [`git`](https://git-scm.com/downloads/mac). After that, you can work from the terminal using [git commands](https://git-scm.com/docs).

If you're not quite ready for that, I suggest you install `Git` by downloading and installing [`GitHub Desktop`](https://desktop.github.com). `GitHub`'s desktop application is a nice GUI and, naturally, integrates well into the repository workflow on `GitHub`. When installed, you can go to `GitHub Desktop > Install Command Line Tool`. After a reboot, all should be set.
...
If you're reading this, first of all, congratulations. You have my respect. I don't know any GUI-based tools for `Git`, so let's stick to the classics.
Install [`Git`](https://git-scm.com/downloads/linux).
After that, you can work from the terminal using [git commands](https://git-scm.com/docs).

------------------------------------------------------------------------

### Command line interface vs. GUI

Ultimately, you'll want to learn how to use `Git` through the command line interface (CLI). It offers better and more comprehensive functionality. Again, [take this 15-minute course](https://learngitbranching.js.org) to get a gentle introduction. 
Later we'll explore in detail how to handle `Git` when things go wrong. You'll be a CLI-wizard by then.
...
When you create a new repository on GitHub, you have the option of creating a default .gitignore file for your language of choice.

------------------------------------------------------------------------

### Linking `GitHub` and `RStudio`

**Securely** linking your local machine to the remote repository is vital when collaborating with other people. You would not want a potential hacker to have contributor access to any of your projects. [This walkthrough video](https://www.gerkovink.com/oldmarkup/Wk1/Link%20Git%20and%20RStudio.mp4) details the process of linking `GitHub` to your machine and `RStudio`. Below we explain the rationale of using both an SSH key and a personal access token.

If you still experience problems after following the aforementioned walkthrough, check [this chapter](https://happygitwithr.com/troubleshooting.html)
...
Because `GitHub` requires Two-Factor Authentication (2FA), your username and password are not sufficient to `push` `commits` to `GitHub` through `RStudio`. To solve this follow these steps on [github.com](https://github.com) as detailed in [this walkthrough video](https://www.gerkovink.com/oldmarkup/Wk1/Link%20Git%20and%20RStudio.mp4):
...
7.  Select at least the `repo` scope; you'll need these permissions to access repositories. Also choose a relevant expiration: the longer the more convenient, the shorter the safer. You can always generate a new token if you need to.
8.  Click Generate token

Copy the token. The token **will not be displayed again**, so paste it directly where it is needed. Remember that you can always generate a new token if you need to.
...
6.  Send a pull request (PR) to incorporate your changes into the `upstream/main` branch (i.e. `mlarpis/markup@main`). You can do this by going to your fork on `GitHub` and clicking the `Pull request` button. Make sure to select the correct branches.
...
This week we will also learn how to use `git` properly when we mess up. But let's not learn that on this year's repo ;-). Fork [this older course repository](https://github.com/mlarpis/markup2020), deselect `Copy the master branch only` in order to copy all branches.

------------------------------------------------------------------------

### Disclaimer

I'll take you by the hand through the exercise. So if you follow this manual step by step, you'll be done at the end and you will have a good notion of troubleshooting with `Git`.
...
Oh, and we forgot to tell you: you'll need the terminal for this exercise. You may use the one in `RStudio`, or the dedicated terminal on UNIX/Linux/Mac or the command prompt or [Windows Terminal](https://www.microsoft.com/en-us/p/windows-terminal/9n0dx20hk701?activetab=pivot:overviewtab) on Windows.
...
1.  Use the terminal to navigate to the `Git` repository (the location of the git repo on your computer)
...
My local `Exercise_wk4` is up to date with `origin/Exercise_wk4`, meaning that GitHub and my local copy are in sync. `origin` is a shorthand name for the remote repository that a project was originally cloned from. It is useful that `origin` is referenced instead of the original repo's URL --\> This makes referencing much more convenient.
Note that this *does not mean* that they will remain in sync automatically from now on. Only that there is a reference between the two. But you still need to `push` and `pull` to keep them in sync in the future.
...
From this `--pretty=oneline` view it is now easy to spot the origin of our course repository - the initial commit that initiated the `Git` goodness. You can also see that Gerko updated the `.gitignore`, a hidden file that specifies which files and directories to be ignored by `Git`. And he did some cosmetic changes and file deletions. This is very insightful because of the detailed, but concise commit messages.
...
-   HEAD: *The HEAD in `Git` is the pointer to the current branch reference. It is the last commit you have made or \[in our case\] checked out.* **REMEMBER that the head will always serve as the parent to the next commit**
...
We now have checked out an old state and made it into a new branch. The cool thing is that we can now commit all we want and everything will be properly logged. Just remember that if you'd like to go back in time and play around with an old state of your project --\> **Merge the old state into a new branch** as demonstrated above.
...
We can always undo a commit. For example, the following command reverts all changes to the initial commit
...
**NOTE: Do not wait too long to undo your resets. After a few days/weeks garbage collection may kick in and `Git` will eventually see old commits as unreferenced. This makes them irretrievable**
...
We've now effectively created a conflict. A conflict must be resolved before a merge is possible. If you switch to GitHub desktop, you'll see this pop-up
...
8.  Post an issue in your forked template (remember [What to hand in](#sec-deliv)), wherein you properly [reference](https://docs.github.com/en/get-started/writing-on-github/working-with-advanced-formatting/autolinked-references-and-urls) your PR to the old course repo.

You should now have all the skills to fill your template with the course deliverables and exercises. Now would be a good time to update your deliverables repository. If you have any questions, please post them [as an issue to the course repository](https://github.com/mlarpis/markup/issues).
...
```{markdown}
% git checkout -b Wk3_pablo
Switched to a new branch 'Wk3_pablo'
% git mv add_text_file.txt pablo.txt
% git commit -m 'pablo`s commit'
[Wk3_pablo 0211826] pablo`s commit
 1 file changed, 0 insertions(+), 0 deletions(-)
 rename add_text_file.txt => pablo.txt (100%)
% git push --set-upstream origin Wk3_pablo
Enumerating objects: 14, done.
Counting objects: 100% (14/14), done.
Delta compression using up to 8 threads
Compressing objects: 100% (7/7), done.
Writing objects: 100% (9/9), 876 bytes | 876.00 KiB/s, done.
Total 9 (delta 5), reused 2 (delta 1), pack-reused 0
remote: Resolving deltas: 100% (5/5), completed with 2 local objects.
remote: 
remote: Create a pull request for 'Wk3_pablo' on GitHub by visiting:
remote:      https://github.com/PabloMosUU/markup2020/pull/new/Wk3_pablo
remote: 
To github.com:PabloMosUU/markup2020.git
 * [new branch]      Wk3_pablo -> Wk3_pablo
branch 'Wk3_pablo' set up to track 'origin/Wk3_pablo'.
```

Note that `git mv` both renames the file and stages the changes.
:::
