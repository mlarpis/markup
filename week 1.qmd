...
`TeX` is widely used by publishers as it gives the user full control about the appearance of the document. It is designed to *write* in such a way that a minimum of attention is required by the user to typeset the document (as opposed to e.g. ~~typesetting hell~~ MS Word). 

---

### Installing a `TeX` framework

I suggest you use RStudio or [Overleaf](https://www.overleaf.com). Note that Overleaf requires internet access. 
RStudio comes with an excellent (but basic) editor; if you want to go the fancy way, there are [very good alternatives](https://en.wikipedia.org/wiki/Comparison_of_TeX_editors). 
If you use Overleaf, please pay attention to any data privacy regulations. It may not be an option for all your research endeavors. 

#### Installing packages

On Overleaf, most packages are installed by default. If you are using RStudio, you might come across packages that you want to use but are not installed by default. To install them, use this command in the RStudio console:
...
[Overleaf](https://www.overleaf.com) provides a series of documents and walkthroughs about `TeX` and `LaTeX`. I suggest that you go throught the following chapters:

1. [Learn `LaTeX` in 30 minutes](https://www.overleaf.com/learn/latex/Learn_LaTeX_in_30_minutes)
2. [Bibliography management with biblatex](https://www.overleaf.com/learn/latex/Bibliography_management_in_LaTeX)
...
If you still across new notation conventions and would like to know how to write expressions with them in `LaTeX`, use [`detexify`](https://detexify.kirelabs.org/classify.html) to draw the symbol and obtain the `LaTeX` code. 
...
Create a document that contains items 1 thru 5 (see below) and then change the document with item 6. Submit both documents as well as the `LaTeXdiff`.
...
1. Fork the template repository detailed in Deliverables. 
2. Add your files to the respective folder.
3. Share your repository link as an issue in [the course repository](https://github.com/mlarpis/markup).
